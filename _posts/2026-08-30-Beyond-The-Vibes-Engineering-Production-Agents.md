...
Which camp is true? Both camps are true, _from their perspective_! Let us explore this in a bit and peel off the layers of agent development. _Camp 1_ approach is suitable for personal productivity and for experimentation / feasibility analysis. _Camp 2_ appraoch is needed for business-critical systems.
...
The next step involves creating a high level design. This step will explain the different systems that the agent is supposed to access, a high level data flow the agent may access, the tools required by the agent, the security guardrails, and constrains on output.
...
After the agent is deployed, we defintely need to monitor for performance, accuracy and drift. If the agents were built with certain assumptions and the underlying assumptions change, the model output could be less relevant. Having a mechanism to detect and alert on failures is critical to agent deployment.
